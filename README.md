...
<img src="common/images/reward_sched_non-term.png" width="380">
...
In order to help speed up the model learning the underlying draft structure, the first FC layer is given several supplimentary boolean inputs corresponding to which positions have been filled in the draft as well as if the draft is currrently in a banning phase.<sup>2</sup>

<sup>2</sup>This isn't strictly necessary since the value of these variables is directly inferrable from the state matrix, but doing so substantially reduces the amount of training time required before the model begins producing legal submissions for the input states.
