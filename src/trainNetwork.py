...
def trainNetwork(Qnet, training_matches, validation_matches, num_epochs, batch_size, buffer_size, load_model = False, verbose = False):
    ...
    with tf.Session() as sess:
        if load_model:
            ...
            sess.run(Qnet.init)
        # First look at what sort of values the network is initialized to:
            ...
            for match in shuffled_matches:
                ...
                for experience in experiences:
                    ...
                    if(True):#if(random.random() < epsilon):
                        ...
                        # Ideally we would let the network predict a random action and evaluate the reward
                        # for the resulting state, but it's not clear how to generate a reward for an action
...
#                        if(state.getAction(*a)!=pred_act):
            ...
            if(verbose):
                print(" Finished epoch {}/{}: dt {:.2f}, mem {}, loss {:.6f}, train {:.6f}, val {:.6f}".format(i+1,num_epochs,t1,total_steps+null_action_count,loss,train_acc,val_acc))
